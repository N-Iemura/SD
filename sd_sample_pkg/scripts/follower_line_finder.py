...
class Follower:
	...
	def image_callback(self, msg):
		...
		#黄色い停止線の検出
		hsv2 = cv.cvtColor(image2, cv.COLOR_BGR2HSV)	#色空間の変換(BGR→HSV)
		lower_yellow1 = np.array([0, 64, 0])		#黄色の閾値（下限）
		upper_yellow1 = np.array([30, 255, 255])	#黄色の閾値（上限）
		mask1 = cv.inRange(hsv2, lower_yellow1, upper_yellow1)	#閾値によるHSV画像の2値化（マスク画像生成）
		masked_yellow = cv.bitwise_and(image2, image2, mask = mask1)
		...
		display_mask_yellow = cv.resize(mask1, RESIZE)
